<template>
  <div class="v-TradHome">
    <trad-area :buy='buy' :sell='sell' :hqList="hqList" :bList='bList' v-on:listenHq="listnBid" :areaIndex="areaIndex" :jFloal="jFloal"></trad-area>
...
<script>
  ...
  // import tradCurrency from '../tradCurrency/tradCurrency.vue'
  export default {
    name: 'tradHome',
    props: {
    ...
    created() {
      ...
      } else if (localStorage.getItem("FirstId1")) {
        this.btype = Number(localStorage.getItem("FirstId1"));
      } else {
        this.FetchGet(this.requestUrl.tradingCenter.bList)
          .then(res => {
            ...
            if (res.data['CNHA']) {
              ...
              this.areaIndex=Number(res.data['CNHA'][0].sorts)-1;
            }
            if (res.data['ETH']) {
    ...
    methods: {
      init() {
        ...
        //深度
        this.pollSend(2000)  //深度
        this.sockePrice(3000)  //行情
      ...
      getBlist() {
        ...
        this.FetchGet(this.requestUrl.tradingCenter.bList)
          .then(res => {
            //_this.Blist = res
            _this.bList = res.data['CNHA'];
            _this.jFloal = Number(res.data.CNHAjfloal);
            if (res.data['CNHA']) {
              if (localStorage.user) {
                // var bIndex = this.btype -2 || 0
                for (let i = 0; i < _this.bList.length; i++) {
                  if (_this.btype == _this.bList[i].id) {
                    bIndex = i;
                  }
                }
                var obj = res.data['CNHA'][bIndex]
                this.goods = res.data['CNHA'][bIndex].goods;
                if (obj.goldnum) {
                  _this.buySum = Number(obj.goldnum)
                  // console.log(obj.goldnum.delFixed(obj.btrade))
                } else {
                  _this.buySum = 1
                }
        ...
        this.FetchGet(this.requestUrl.tradingCenter.hqList, params)
          .then(res => {
            ...
            if (_this.last) {
              ...
              //last 第一次出来
              this.titleCallback(_this.hqList.last, this.goods);
              _this.last = _this.hqList.last;
            }
            if (callback) {
              callback(_this.last)
      ...
      getRealList(id) {
        ...
        this.FetchPost(this.requestUrl.tradingCenter.RealList, { typeboxid: this.btype })
          .then(res => {
            //_this.Blist = res
      ...
      getKlineList(id) {
        ...
        this.FetchPost(this.requestUrl.tradingCenter.KineList, { typeboxid: this.btype, time: time })
          .then(res => {
    ...
    watch: {
      bList() {//
        ...
        if (id) {
          ...
          this.btype = Number(localStorage.getItem("btypeId")) || Number(this.bList[0].id);
        }
      },
      goods(newVal, oldVal) {
        this.titleCallback(this.last, this.goods)
